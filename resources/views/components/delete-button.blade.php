...
<div x-data="{ initial: true, deleting: false }" class="text-sm flex items-center">
    <button
        ...
        class="btn btn-sm btn-error text-white p-1 rounded bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 disabled:opacity-50"
    >
        {{ $buttonText }}
    </button>
    ...
    >
        <form x-on:submit="$dispatch('deleting')" method="post" action="{{ $action }}">
            ...
            <button
                ...
                x-on:deleting.window="$el.disabled = true"
                class="btn btn-sm text-white p-1 rounded bg-gray-600 hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 disabled:opacity-50"
            >
                @lang('No')
            </button>
        </form>
